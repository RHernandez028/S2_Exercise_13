...
function init() {
      ...
      // removing inline backgroudColor style from each cell
...
function swapPuzzle (e) {
      ...
      //retrieved the value of the clicked button
...
function setupPuzzle() {
      ...
      var filled = document.querySelectorAll("tabel#hanjieGrid td.filled");
      var empty = document.querySelectorAll("tabel#hanjieGrid td.empty");
...
function setBackground(e) {
      ...
      //set the background based on the keyboard key that is pressed
      if (e.shiftKey){
      ...
      //create an event lister for every puzzle cell
      for (var i = 0; i < puzzleCells.length; i++ ) {
            puzzleCells[i].addEventListener("mouseenter", extendBackground);
            puzzleCells[i].style.cursor = cursorType;
      }

      // prevetn the default action of selection table text
      e.preventDefault();
}

//creat a function to extend the backgrund and set a new background color
function extendBackground(e) {
      e.target.style.backgroundColor = cellBackground;
}

function endBackground() {
      //remove the event listner for evry puzzle cell
      for (var i = 0; i < puzzleCells.length; i++ ) {
